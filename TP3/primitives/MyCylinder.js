...
class MyCylinder extends CGFobject {
    ...
    initBuffers() {
        ...
        for (let i = 0; i <= this.stacks; i++) {
            let trueRadius = i / this.stacks * (this.bottomRadius - this.topRadius) + this.topRadius; //Radius of stack i

            for (let j = 0; j <= this.slices; j++) {
                let ang = j / this.slices * Math.PI * 2;
        ...
        for (let i = 0; i < this.slices; i++) {
            for (let j = 0; j < this.stacks; j++) {
                let vert1 = a + (j) * (this.slices + 1) + i;
                let vert2 = a + (j) * (this.slices + 1) + i + 1;
                let vert3 = a + (j + 1) * (this.slices + 1) + i;
                let vert4 = a + (j + 1) * (this.slices + 1) + i + 1;
